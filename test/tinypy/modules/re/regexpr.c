                                   ...
                                   unsigned char *fastmap)
{
	...
	for (;;)
		...
		default:
		{
		        /*PyErr_SetMpStr(PyExc_SystemError, "Unknown regex opcode: memory corrupted?");*/
...
static int re_optimize_star_jump(regexp_t bufp, unsigned char *code)
{
	...
	if (p1<bufp->buffer || bufp->buffer+bufp->used<p1)
	  {
	    /*PyErr_SetMpStr(PyExc_SystemError, "Regex VM jump out of bounds (failure_jump opt)");*/
  ...
  continue_matching:
	...
	case Cjump:
	{
		...
		if (code<bufp->buffer || bufp->buffer+bufp->used<code) {
			/*PyErr_SetMpStr(PyExc_SystemError, "Regex VM jump out of bounds (Cjump)");*/
			re_errno = TP_RE_JUMP_OUT_BOUNDS;
			FREE_STATE(state);
			return -2;
		}
	...
	case Cdummy_failure_jump:
	{
		...
		if (failuredest<bufp->buffer || bufp->buffer+bufp->used < failuredest) {
			/*PyErr_SetMpStr(PyExc_SystemError, "Regex VM jump out of bounds (Cdummy_failure_jump failuredest)");*/
			re_errno = TP_RE_JUMP_OUT_BOUNDS;
			FREE_STATE(state);
			return -2;
		}
		...
		code += a;
		if (code<bufp->buffer || bufp->buffer+bufp->used < code) {
			/*PyErr_SetMpStr(PyExc_SystemError, "Regex VM jump out of bounds (Cdummy_failure_jump code)");*/
			re_errno = TP_RE_JUMP_OUT_BOUNDS;
			FREE_STATE(state);
			return -2;
		}
	...
	case Cfailure_jump:
	{
		...
		if (code+a<bufp->buffer || bufp->buffer+bufp->used < code+a) {
			/*PyErr_SetMpStr(PyExc_SystemError, "Regex VM jump out of bounds (Cfailure_jump)");*/
			re_errno = TP_RE_JUMP_OUT_BOUNDS;
			FREE_STATE(state);
			return -2;
		}
	...
	case Crepeat1:
	{
		...
		if (pinst<bufp->buffer || bufp->buffer+bufp->used<pinst) {
			/*PyErr_SetMpStr(PyExc_SystemError, "Regex VM jump out of bounds (Crepeat1)");*/
			re_errno = TP_RE_JUMP_OUT_BOUNDS;
			FREE_STATE(state);
			return -2;
		}
