...
def print_xml_obj(tree, n=0):
    if tree == None:
        return
    if tree.type in ("number", "string", "None"):
        print(" " * n, xml_obj(tree.type, tree.val))
        return
    if tree.type == "name":
        print(" " * n, xml_obj("name", tree.val))
        return
    else:
        print(" " * n, xml_start(tree.type))
    if hasattr(tree, "first"):
        print_xml(tree.first, n+2)
    if hasattr(tree, "second"):
        print_xml(tree.second, n+2)
    if hasattr(tree, "third"):
        print_xml(tree.third, n+2)
